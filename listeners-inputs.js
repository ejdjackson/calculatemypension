...
/* function toggleTable(element) {
    ...
    function outputResults(cashFlowData, todaysMoneyCashFlowData, currentAge, retirementAge, fundAtRetirement, ISAAtRetirement, taxFreeCashTaken, desiredAnnualIncome, maxAffordableNetIncome, shortfallAtRetirement, discountFactor, alreadyRetired, planAsCouple, phoneFormat, tabletFormat) {
       ...
       /*  if (applyInflationAdjustment) {
        ...
        document.getElementById("inputTFCTakenLabel").innerHTML = `<strong>Tax Free Amount Received at age ${retirementAge}:   </strong>`;
        document.getElementById("inputTFCTaken").innerHTML = '<strong>£' + formatNumber(Math.round(taxFreeCashTaken / 10) * 10) + '</strong>';   

        document.getElementById("resultsExplainer").innerHTML = `Allowing for the values of all your inputs, the retirement income that you can afford from retirement at age ${retirementAge} up to a projection-end age of ${endAge}, allowing for state pension payble from age ${statePensionAge} has been calculated to be:`;
